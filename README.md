...
**Result.** At 100k (frozen F1 probe, historical submitted evidence) the experiment reached 100,000 viewers with zero correctness violations but measured fan_out p95 2757 ms and burst p95 3707 ms (F1 short probe), missing the frozen 500/1000 ms gates; **M3 was hard-stopped without ACCEPT**. A subsequent accept-push (2026-08-24) identified B1 (p0-only backup, p1/p2/p3 distributed) as the best validated configuration: under fair long-window (section-10 bridge) measurement fan_out p95 4242 ms (2.6x better than F1's 11200 ms under the same windows) with duplicates=0, avoiding F1's burst duplicate-correctness failure — still far above the frozen gates, so config-only ACCEPT remains unachievable. The reduced reviewer run reproduces the same measurement path and emits fan_out p95 at its smaller scale (NOT the 100k failure, and not a re-run of M3).

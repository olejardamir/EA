...
1. all hard requirements have at least one credible satisfying architecture;
2. remaining alternatives are dominated or differ only in low-impact implementation detail;
3. the preferred architecture survives the important composition checks;
4. its most consequential unresolved assumption is clear enough to test;
5. further alternatives would add breadth without changing the decision.

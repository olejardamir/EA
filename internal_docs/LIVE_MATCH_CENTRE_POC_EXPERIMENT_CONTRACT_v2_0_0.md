...
**Date:** 2026-08-19
**Status:** FROZEN
**Supersedes:** the earlier raw-WebSocket experiment contract (never implemented)
...
│
├── publisher
│   TypeScript deterministic sports-event simulator
│   publishes to nchan:8080 via HTTP POST
│   deploy.resources.limits: cpus=2, memory=1G
│
├── loadgen-primary
│   TypeScript SSE load generator
│   connects to nchan:8081 via EventSource
│   handles steady-state fan-out measurement
│   deploy.resources.limits: cpus=4, memory=4G
│
├── loadgen-surge
│   TypeScript SSE load generator
│   handles the +40,000/120s surge ramp test
│   deploy.resources.limits: cpus=4, memory=4G
│
└── aggregator
    TypeScript result aggregation + final summary
    deploy.resources.limits: cpus=2, memory=2G
```

Total container resource envelope: **18 CPUs, 17 GB RAM**. The host must have at least this much available.

Multiple load-generator containers are permitted and expected because a single Node.js process may saturate before Nchan does.
...
```nginx
# Subscriber endpoint for SSE
location /sub/match_ {
    nchan_subscriber;
    nchan_subscriber_first_message oldest;
    nchan_channel_id $slice(5, $request_uri);
    nchan_eventsource;
    nchan心跳 interval 15s;
    nchan_subscriber_message_buffer_length 0;
    nchan_subscriber_connection_pool_size 256;
}
```
...
location /sub/lobby {
    ...
    nchan_eventsource;
    nchan心跳 interval 15s;
    nchan_subscriber_message_buffer_length 1;
}
```
...
```nginx
nchan_redis_storage_url "redis://redis:6379";
nchan_use_redis on;
nchan_redis_connect_timeout 5000;
nchan_redis_read_timeout 10000;
```

### Channel buffering (per match channel)

```nginx
# For match channels: retain full active-match history in Redis
# nchan message timeout = 0 means messages do not expire while channel is active
nchan_message_timeout 0;
nchan_message_buffer_length 0;
# 0 = unbounded; the active match retains everything until match ends
# This is acceptable because the assignment caps at 8 concurrent matches
```

### Heartbeat

SSE heartbeat interval: **15 seconds**. This keeps CloudFront/intermediaries from closing idle connections (CloudFront default origin read timeout is 30s; 15s heartbeat is safe).
...
First message for new subscriber: newest (current state)
```

Lobby viewers receive:
1. Current state immediately on connect (newest-first).
...
This is conservative: a 90-minute match at ~1.25 events/s produces ~6,750 events. At burst rates, a match could accumulate faster. 5,000 represents a realistic active-match buffer that the POC must be able to replay.
...
| redis | 2 | 2 GB |
| publisher | 2 | 1 GB |
| loadgen-primary | 4 | 4 GB |
| loadgen-surge | 4 | 4 GB |
| aggregator | 2 | 2 GB |
| **Total** | **18** | **17 GB** |

These are Docker `deploy.resources.limits`. The host must have at least 20 CPUs and 20 GB RAM available for Docker.
...
Because publisher and generator may be in different containers, clock drift is expected to be negligible (sub-millisecond on the same host). The POC must document the measured clock offset if measurable, or acknowledge that cross-container timing has a small不确定性.
...
```text
internal_docs/LIVE_MATCH_CENTRE_POC_EXPERIMENT_CONTRACT_v2_0_0.md
```

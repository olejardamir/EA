...
## 5.1 Gorilla WebSocket — RECOMMENDED FOR POC
...
- Autobahn protocol-tested;
- `PreparedMessage` caches the wire representation for efficient repeated broadcast;
- write-buffer pools reduce per-connection memory pressure;
- the official chat example uses a hub + per-client buffered output queue and disconnects stuck clients when buffers fill.

That maps closely to our required behavior:
...
-> reconnect/replay from durable state
```

### Recommendation

Use Gorilla WebSocket rather than implementing the WebSocket protocol ourselves.

For the fan-out path:
- serialize event once;
- build one prepared message;
- enqueue/send that same prepared message to subscribers;
- bound per-client queues;
- count slow-client drops;
- measure memory with and without buffer pooling if useful.
...
A lightweight Go load generator in a separate container may give us tighter control over:
- connection count;
- file descriptors;
- ephemeral ports;
...
- Gorilla WebSocket Go API: https://pkg.go.dev/github.com/gorilla/websocket
...
POC implementation direction refined: YES
